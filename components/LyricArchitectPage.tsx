    ...
    <div className="space-y-8 animate-in fade-in duration-500">
      <div className="flex flex-col gap-2">
        ...
        <div className="flex items-center justify-between">
          <h2 className="text-3xl font-extrabold text-white">Lyric Architect</h2>
          <button 
            onClick={clearAll}
      ...
      <div className="p-6 bg-slate-800/40 rounded-3xl border border-slate-700/50 space-y-6">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div>
            <label className="block text-xs font-bold text-slate-500 uppercase mb-2">가사 제목</label>
            <input 
              type="text" 
              value={draft.title}
              onChange={(e) => updateDraft({ title: e.target.value })}
              placeholder="가사의 제목을 입력하세요..."
              className="w-full bg-slate-900/60 border border-slate-700/50 rounded-xl px-4 py-2 text-sm text-slate-200 focus:outline-none focus:ring-2 focus:ring-indigo-500/30"
            />
          </div>
          <div>
            <label className="block text-xs font-bold text-slate-500 uppercase mb-2">아티스트 스타일 적용 (선택)</label>
            <select 
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        <div className="space-y-4">
          <label className="block text-xs font-bold text-slate-500 uppercase px-1">원문 가사</label>
          <textarea
            value={draft.raw}
            onChange={(e) => updateDraft({ raw: e.target.value })}
            placeholder="여기에 가사 원고를 입력하세요..."
            className="w-full h-[450px] bg-slate-800/30 border border-slate-700/50 rounded-3xl p-6 text-slate-200 focus:outline-none focus:ring-2 focus:ring-indigo-500/30 transition-all resize-none leading-relaxed"
          />
          <button
            onClick={handleStructure}
            disabled={!draft.raw.trim() || isLoading}
            className="w-full py-4 bg-indigo-600 hover:bg-indigo-500 disabled:bg-slate-700 text-white rounded-2xl font-bold text-lg flex items-center justify-center gap-3 transition-all shadow-lg shadow-indigo-500/10"
          >
        ...
        <div className="space-y-4">
          <label className="block text-xs font-bold text-slate-500 uppercase px-1">구조화된 결과</label>
          <div className={`w-full h-[450px] bg-black/40 border border-slate-700/50 rounded-3xl p-6 text-blue-300 font-mono text-sm leading-relaxed overflow-y-auto whitespace-pre-wrap ${!draft.structured && 'flex items-center justify-center text-slate-600'}`}>
          ...
          <div className="grid grid-cols-2 gap-4">
            <button
              ...
              className="py-4 bg-slate-700 hover:bg-slate-600 disabled:opacity-50 text-white rounded-2xl font-bold transition-all"
            >
              클립보드 복사
            </button>
            <button
              onClick={handleSave}
              disabled={!draft.structured}
              className="py-4 bg-emerald-600 hover:bg-emerald-500 disabled:opacity-50 text-white rounded-2xl font-bold transition-all"
            >
              내 저장소에 저장
            </button>
          </div>
